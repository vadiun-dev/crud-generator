...
use Hitocean\CrudGenerator\FileGenerators\Controller\ModelController\ModelControllerUpdateMethod;
...
it('creates a store method for the controller test', function ()
{
    ...
    $method = ModelControllerUpdateMethod::create($inputs, 'store', 'TestModel', $class, 'Src\\Models\\TestModel');
    ...
    expect($method->getName())->toBe('store')
        ...
        ->and($method->getParameters()['data']->getType())->toBe('Src\Models\TestModel')
                              ->and($methodBody)
                              ->toContain('$model = TestModel::findOrFail($data->id);')
                              ->toContain('$model->update([')
